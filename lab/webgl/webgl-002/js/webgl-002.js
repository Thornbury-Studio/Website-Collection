...
(function(){
    const wrap = $("#cylWrap"), stack = $("#cylStack");
    const LINES = ["A site is a room","not a brochure","it has a door","it has a floor",
                   "it has weather","people arrive in it","some of them stay","that is the whole job"];
    const N = LINES.length, STEP = 360/N;
    const radius = Math.round(Math.min(innerWidth, 900) * 0.30);
    LINES.forEach((t,i)=>{
      const s = document.createElement("span");
      s.textContent = t;
      s.style.transform = "rotateX(" + (i*STEP) + "deg) translateZ(" + radius + "px)";
      stack.appendChild(s);
    });
    const spans = $$("span", stack);
    live(wrap, ()=>{
      const p = pin(wrap);
      const rot = -p * 360 * ((N-1)/N);
      stack.style.transform = "rotateX(" + rot.toFixed(2) + "deg)";
      /* fade the faces pointing away so it reads as a solid barrel */
      spans.forEach((s,i)=>{
        const face = ((i*STEP + rot) % 360 + 360) % 360;
        const front = Math.cos(face * Math.PI/180);
        s.style.opacity = clamp(front, 0, 1).toFixed(3);
        s.style.color = front > .82 ? "var(--ink)" : "#9c9890";
      });
    });
  })();
})();
