...
(function(){
    const sys = $("#orbSys"); if (!sys) return;
    const ORBITS = [
      {r:60, tilt:70, speed:0.9, size:8},
      {r:90, tilt:55, speed:0.6, size:9},
      {r:120,tilt:80, speed:0.4, size:7},
      {r:150,tilt:40, speed:0.28,size:10},
      {r:185,tilt:65, speed:0.2, size:8}
    ];
    const dots = ORBITS.map(o=>{
      const path = document.createElement("div");
      path.className = "orb-path";
      path.style.transform = "rotateX(" + o.tilt + "deg)";
      const dot = document.createElement("div");
      dot.className = "orb-dot";
      dot.style.width = dot.style.height = o.size + "px";
      path.appendChild(dot);
      sys.appendChild(path);
      return {path, dot, o};
    });
    live(sys.closest(".orb-stage"), (t)=>{
      dots.forEach(({dot,o})=>{
        const a = (t/1000) * o.speed;
        dot.style.transform = "translate(" + (Math.cos(a)*o.r - o.size/2) + "px," + (Math.sin(a)*o.r - o.size/2) + "px)";
        dot.style.opacity = String(0.72 + 0.28*Math.sin(a*2));
      });
      sys.parentElement.style.transform = "rotateY(" + (t*0.006) + "deg)";
    });
  })();
})();
