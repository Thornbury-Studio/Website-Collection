...
(function(){
    const track = $("#veloTrack"), read = $("#veloRead"), host = track.parentElement;
    const WORDS = ["AUDITED","DESIGNED","BUILT","HANDED OVER","OWNED"];
    for (let pass=0; pass<2; pass++){
      WORDS.forEach((w,i)=>{
        const s = document.createElement("span");
        s.textContent = w + " — ";
        if (i % 2) s.className = "o";
        track.appendChild(s);
      });
    }
    let last = scrollY, vel = 0, x = 0;
    addEventListener("scroll", ()=>{ vel += (scrollY - last); last = scrollY; }, {passive:true});
    live(host, ()=>{
      vel *= 0.90;
      const half = track.scrollWidth/2 || 1;
      x = (x - 0.7 - vel*0.85) % half;
      if (x > 0) x -= half;
      const skew = clamp(-vel*0.16, -11, 11);
      track.style.transform = "translateX(" + x.toFixed(1) + "px) skewX(" + skew.toFixed(2) + "deg)";
      read.textContent = "scroll velocity — " + Math.abs(vel).toFixed(1) + " px/frame" +
                         (Math.abs(vel) < .4 ? " (idle drift)" : vel > 0 ? " ↓ running left" : " ↑ running right");
    });
  })();
})();
