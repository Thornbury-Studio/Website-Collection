...
(function(){
    const wrap = $("#dwWrap"), A = $("#dwA"), B = $("#dwB");
    const LEFT  = ["measured","drawn","built","handed over","owned","measured again"];
    const RIGHT = ["not <em>guessed</em>","not picked","not assembled","not held back","not rented","not forgotten"];
    LEFT.forEach(t=>{ const s=document.createElement("span"); s.textContent=t; A.appendChild(s); });
    RIGHT.forEach(t=>{ const s=document.createElement("span"); s.innerHTML=t; B.appendChild(s); });
    const a = $$("span",A), b = $$("span",B);
    live(wrap, ()=>{
      const p = pin(wrap);
      const drive = p * Math.PI * 2.4;
      a.forEach((s,i)=>{
        const ph = drive - i*0.55;
        s.style.transform = "translateX(" + (Math.sin(ph)*9).toFixed(2) + "vw)";
        s.classList.toggle("lit", Math.cos(ph) > 0.72);
      });
      b.forEach((s,i)=>{
        const ph = drive - i*0.55;
        s.style.transform = "translateX(" + (-Math.sin(ph)*9).toFixed(2) + "vw)";
