...
(function(){
    const host = $("#prows");
    const NAMES = ["MIDWATER","KIYO 清","AUREL","LOAM","FORM/01","UNSTILL","NEAP",
                   "MERIDIAN","AMICUS","PULSE","VELA","ATELIER № 8","OSCILLA","NOCTURNE"];
    const rows = $$(".prow", host);
    rows.forEach((row,ri)=>{
      /* twice through, so the loop can wrap on half the width */
      for (let pass=0; pass<2; pass++){
        NAMES.forEach((n,i)=>{
          const p = document.createElement("div");
          p.className = "pl";
          const pal = PALETTES[(i+ri*2) % PALETTES.length];
          p.style.background = "linear-gradient(150deg," + pal[0] + "," + pal[1] + " 62%," + pal[2] + ")";
          p.textContent = n;
          row.appendChild(p);
        });
      }
    ...
    live(host, (t)=>{
      ...
      const through = (innerHeight - r.top) / (innerHeight + r.height);
      rows.forEach(row=>{
        const speed = parseFloat(row.dataset.speed);
        const half = row.scrollWidth/2;
        if (!half) return;
        let x = (t*0.026*Math.sign(speed) + through*speed*2600) % half;
        if (x > 0) x -= half;
