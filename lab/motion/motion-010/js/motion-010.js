...
(function(){
    const stage = $("#blind"), cv = $("canvas", stage), slats = $("#blindSlats");
    const N = 12;
    for (let i=0;i<N;i++){
      const s = document.createElement("i");
      s.style.transformOrigin = i%2 ? "50% 100%" : "50% 0%";
      s.style.transition = "transform .8s cubic-bezier(.76,0,.24,1)";
      s.style.transitionDelay = (i*55) + "ms";
      slats.appendChild(s);
    }
    const bars = $$("i", slats);
    once(stage, ()=>{
      const {ctx,w,h} = fit(cv, 1.5);
      plate(ctx,w,h,0);
      requestAnimationFrame(()=> bars.forEach(b=> b.style.transform = "scaleY(0)"));
    });
  })();
})();
