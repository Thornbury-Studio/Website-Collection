...
(function(){
    const nav = $("#enav"), bar = $(".bar", nav);
    const links = $$("a", nav);
    let current = links[0], settle = 0;
    function place(a, animate){
      const nr = nav.getBoundingClientRect(), r = a.getBoundingClientRect();
      const from = parseFloat(bar.style.left) || 0;
      const to = r.left - nr.left;
      bar.style.left = to + "px";
      bar.style.width = r.width + "px";
      bar.style.top = (r.bottom - nr.top - 2) + "px";
      if (animate){
        /* stretch in proportion to how far it has to travel, then settle */
        bar.style.transform = "scaleX(" + (1 + clamp(Math.abs(to-from)/240, 0, 1.15)) + ")";
        clearTimeout(settle);
        settle = setTimeout(()=>{ bar.style.transform = "scaleX(1)"; }, 190);
      }
    }
    links.forEach(a=>{
    ...
    nav.addEventListener("pointerleave", ()=> place(current, true));
    once(nav, ()=> requestAnimationFrame(()=> place(current, false)), .1);
