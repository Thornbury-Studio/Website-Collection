...
(function(){
    const bar = $("#ctnBar"), frame = $("#ctnFrame"), wipe = $("#ctnWipe");
    const panels = $$(".ctn-panel", frame);
    let cur = 0, busy = false;
    bar.addEventListener("click", e=>{
      const b = e.target.closest("button"); if (!b || busy) return;
      const to = +b.dataset.p; if (to === cur) return;
      busy = true;
      $$("button", bar).forEach(x=> x.setAttribute("aria-pressed", String(+x.dataset.p === to)));
      if (reduce){ swap(to); busy = false; return; }
