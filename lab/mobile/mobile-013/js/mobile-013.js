...
(function(){
    const view = $("#esView"), front = $(".es-front", view||document), edge = $("#esEdge");
    if (!view) return;
    let sx = 0, dragging = false;
    edge.addEventListener("pointerdown", e=>{
      dragging = true; edge.setPointerCapture(e.pointerId); sx = e.clientX;
      front.style.transition = "";
    });
    edge.addEventListener("pointermove", e=>{
      if (!dragging) return;
      const dx = clamp(e.clientX - sx, 0, view.clientWidth);
      front.style.transform = "translateX(" + dx + "px)";
    });
    const release = e=>{
      if (!dragging) return; dragging = false;
      const dx = clamp((e.clientX||sx) - sx, 0, view.clientWidth);
      front.style.transition = "transform .3s cubic-bezier(.16,1,.3,1)";
      if (dx > view.clientWidth*0.38){
        front.style.transform = "translateX(100%)";
        setTimeout(()=>{ front.style.transition = "none"; front.style.transform = "translateX(0)"; }, 320);
      } else {
