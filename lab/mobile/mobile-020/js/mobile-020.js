...
(function(){
    const echo = $("#hpEcho"), status = $("#hpStatus"); if (!echo) return;
    const supported = "vibrate" in navigator;
    status.innerHTML = supported
      ? "haptics — <b>on this browser, real</b>"
      : "haptics — <b>not on this browser</b> (visual only)";
    $$(".hp-btns button").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        const ring = document.createElement("i");
        echo.appendChild(ring);
        requestAnimationFrame(()=> ring.classList.add("go"));
        setTimeout(()=> ring.remove(), 650);
        if (supported){
          const pattern = btn.dataset.pat.split(",").map(Number);
