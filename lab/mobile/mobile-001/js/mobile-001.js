...
(function(){
    const list = $("#swList"); if (!list) return;
    $$(".sw-item", list).forEach(item=>{
      /* the markup already nests .sw-track > .sw-content per row — this
         only has to switch it on, not build it. */
      const track = $(".sw-track", item), content = $(".sw-content", item);
      item.classList.add("is-init");
      requestAnimationFrame(()=>{ track.scrollLeft = track.clientWidth; });
      const activate = 0.8, commit = 0.35;
      const obs = new IntersectionObserver((es)=>{
        ...
        if (ratio < commit){
          ...
          setTimeout(()=>{ item.style.height = "0"; item.style.overflow = "hidden";
            item.style.transition = "height .25s ease"; }, 30);
          obs.disconnect();
          return;
        }
        item.classList.toggle("is-activating", ratio < activate);
      },{ root: track, threshold:[commit, activate] });
      once(item, ()=> obs.observe(content), .01);
    });
  })();
})();
