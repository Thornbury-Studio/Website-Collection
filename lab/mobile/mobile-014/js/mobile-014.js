...
(function(){
    const btn = $("#pcBtn"); if (!btn) return;
    const fill = $(".fill", btn), lbl = $(".lbl", btn);
    let raf = 0, t0 = 0, held = false;
    function frame(t){
      const p = clamp((t - t0) / 1100, 0, 1);
      fill.style.transform = "scaleX(" + p + ")";
      if (p >= 1){
        held = false; btn.classList.add("done"); lbl.textContent = "Draft deleted";
        setTimeout(()=>{
          btn.classList.remove("done"); lbl.textContent = "Hold to delete draft";
          fill.style.transition = "transform .2s ease"; fill.style.transform = "scaleX(0)";
          setTimeout(()=>{ fill.style.transition = ""; }, 210);
        }, 1100);
        return;
      }
      if (held) raf = requestAnimationFrame(frame);
    }
    function start(){ held = true; t0 = performance.now(); fill.style.transition = ""; raf = requestAnimationFrame(frame); }
    function cancel(){
