...
(function(){
    const tile = $("#dtTile"), heart = $("#dtHeart"), badge = $("#dtBadge");
    if (!tile) return;
    let lastTap = 0, n = 0, liked = false;
    function burst(){
      heart.style.transition = "none"; heart.style.opacity = "0"; heart.style.transform = "translate(-50%,-50%) scale(.4)";
      requestAnimationFrame(()=>{
        heart.style.transition = "opacity .6s ease, transform .6s cubic-bezier(.16,1,.3,1)";
        heart.style.opacity = "1"; heart.style.transform = "translate(-50%,-50%) scale(1.15)";
        setTimeout(()=>{ heart.style.opacity = "0"; }, 420);
      });
    }
    tile.addEventListener("pointerdown", ()=>{
      ...
      if (t - lastTap < 320){
        burst();
        if (!liked){ liked = true; n++; badge.textContent = "♥ " + n; }
      }
      lastTap = t;
    });
  })();
