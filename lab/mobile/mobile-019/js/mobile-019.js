...
(function(){
    const view = $("#obView"), list = $("#obList"), cap = $("#obCap");
    if (!view) return;
    let sy = 0, dragging = false, base = 0;
    view.addEventListener("pointerdown", e=>{
      dragging = true; view.setPointerCapture(e.pointerId); sy = e.clientY;
      list.style.transition = ""; base = 0;
    });
    view.addEventListener("pointermove", e=>{
      if (!dragging) return;
      const dy = e.clientY - sy;
      const give = dy * 0.32;
      list.style.transform = "translateY(" + give + "px)";
      cap.classList.toggle("on", Math.abs(give) > 14);
    });
    const release = ()=>{
      if (!dragging) return; dragging = false;
      list.style.transition = "transform .3s cubic-bezier(.34,1.2,.64,1)";
