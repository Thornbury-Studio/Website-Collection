...
(function(){
    const grid = $("#bento");
    const wide = ["1.7fr .72fr .72fr", ".72fr 1.7fr .72fr", ".72fr .72fr 1.7fr"];
    $$(".bento-cell", grid).forEach(c=>{
      c.addEventListener("pointerenter", ()=>{
        if (innerWidth < 760) return;
        grid.style.gridTemplateColumns = wide[(+c.dataset.col) - 1];
      });
    });
    grid.addEventListener("pointerleave", ()=>{ grid.style.gridTemplateColumns = ""; });
  })();
})();
