...
(function(){
    const dock = $("#dockEl"); if (!dock) return;
    const icons = $$("i", dock);
    let mx = -1e4;
    dock.addEventListener("pointermove", e=>{ mx = e.clientX; });
    dock.addEventListener("pointerleave", ()=>{ mx = -1e4; });
    live(dock, ()=>{
      icons.forEach(icon=>{
        const r = icon.getBoundingClientRect();
        const cx = r.left + r.width/2;
        const d = Math.abs(cx - mx);
        const falloff = Math.exp(-(d*d)/(2*70*70));
        const target = 1 + falloff*0.9;
        const cur = parseFloat(icon.dataset.s || "1");
        const next = lerp(cur, target, 0.25);
        icon.dataset.s = String(next);
        icon.style.transform = "scale(" + next.toFixed(3) + ") translateY(" + (-(next-1)*14).toFixed(1) + "px)";
      });
    });
  })();
})();
