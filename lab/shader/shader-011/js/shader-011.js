...
(function(){
    const stage = $(".mir"), cv = $(".ripple", stage);
    let ctx=null, W=0, H=0, ready=false;
    function build(){ const f = fit(cv,1); ctx=f.ctx; W=f.w; H=f.h; ready = true; }
    once(stage, build, .01);
    ...
    live(stage, (t)=>{
      ...
      for (let i=0;i<16;i++){
        ...
        ctx.beginPath();
        for (let x=0;x<=W;x+=8){
          const yy = y + Math.sin(x*0.012 + t/900 + i*0.6)*amp + Math.sin(x*0.031 - t/1400)*amp*.4;
          x === 0 ? ctx.moveTo(x,yy) : ctx.lineTo(x,yy);
        }
        ctx.strokeStyle = "rgba(242,239,230," + (0.012 + (y/H)*0.035).toFixed(3) + ")";
        ctx.lineWidth = 1 + (y/H)*2;
        ctx.stroke();
      }
    });
  })();
