...
(function(){
    const panel = $("#glassPanel"), cv = $("#glassCanvas");
    const map = $("#glassMap"), turb = $("#glassTurb");
    if (!panel) return;
    once(panel, ()=>{
      const f = fit(cv, 1.5);
      plate(f.ctx, f.w, f.h, 1);
    }, .05);
    panel.addEventListener("pointermove", e=>{
      const r = panel.getBoundingClientRect();
      const nx = (e.clientX - r.left)/r.width, ny = (e.clientY - r.top)/r.height;
      map.setAttribute("scale", (10 + Math.hypot(nx-.5,ny-.5)*70).toFixed(1));
      turb.setAttribute("baseFrequency", (0.008 + nx*0.014).toFixed(4) + " " + (0.01 + ny*0.02).toFixed(4));
    });
    panel.addEventListener("pointerleave", ()=> map.setAttribute("scale","6"));
    if (!reduce) live(panel, (t)=>{
      if (panel.matches(":hover")) return;
      turb.setAttribute("seed", String(1 + Math.floor(t/1100) % 30));
    });
  })();
})();
