...
(function(){
    ...
    for (let i=0;i<N;i++){
      ...
      const s = 78 + Math.random()*96;
      b.style.width = s+"px"; b.style.height = s+"px";
      b.style.background = i === 0 ? "var(--accent)" : "var(--ink)";
      host.appendChild(b);
      blobs.push({el:b, s, a:Math.random()*7, r:0.07+Math.random()*0.10, sp:0.00022+Math.random()*0.00050,
                  ox:0.30+Math.random()*0.40, oy:0.30+Math.random()*0.40, x:0, y:0});
    }
    let mx = null, my = null;
    host.addEventListener("pointermove", e=>{
      const r = host.getBoundingClientRect(); mx = e.clientX-r.left; my = e.clientY-r.top;
    });
    host.addEventListener("pointerleave", ()=>{ mx = my = null; });
    live(host, (t)=>{
      const W = host.clientWidth, H = host.clientHeight;
      blobs.forEach((b,i)=>{
        let gx = W*b.ox + Math.cos(t*b.sp + b.a)*W*b.r;
        let gy = H*b.oy + Math.sin(t*b.sp*1.3 + b.a)*H*b.r;
        if (mx !== null){
          /* the leader tracks the hand, the rest are drawn toward it */
          const k = i === 0 ? .16 : .045/(1 + i*0.3);
          gx = lerp(gx, mx, i === 0 ? .18 : k*4);
          gy = lerp(gy, my, i === 0 ? .18 : k*4);
        }
        b.x = clamp(lerp(b.x || gx, gx, .12), b.s/2, W - b.s/2);
        b.y = clamp(lerp(b.y || gy, gy, .12), b.s/2, H - b.s/2);
        b.el.style.transform = "translate(" + (b.x - b.s/2).toFixed(1) + "px," + (b.y - b.s/2).toFixed(1) + "px)";
      });
    });
  })();
})();
