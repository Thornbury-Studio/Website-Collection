...
(function(){
    const stage = $("#bleedStage"), host = $("#bleed");
    const txt = host.textContent;
    host.textContent = "";
    [...txt].forEach(ch=>{
      const s = document.createElement("span"); s.textContent = ch; host.appendChild(s);
    });
    let on = false;
    function toggle(){
      on = !on;
      stage.classList.toggle("merged", on);
      /* no per-glyph mangling — that reads as a rendering fault. The
         tracking closes, and the threshold filter does the bleeding. */
    }
    host.addEventListener("click", toggle);
    stage.addEventListener("click", e=>{ if (e.target === stage) toggle(); });
    /* let it bleed once on arrival so the stage is never inert */
    once(stage, ()=>{ if (reduce) return; setTimeout(toggle, 500); setTimeout(toggle, 2600); });
  })();
})();
