...
(function(){
    const wrap = $("#wtWrap"), line = $("#wtLine");
    const words = line.textContent.trim().split(/(\s+)/);
    line.textContent = "";
    const els = [];
    words.forEach(t=>{
      if (!t.trim()){ line.appendChild(document.createTextNode(t)); return; }
      const w = document.createElement("w");
      w.textContent = t; line.appendChild(w); els.push(w);
    });
    live(wrap, ()=>{
      const p = pin(wrap);
      els.forEach((w,i)=>{
        /* each word has its own slice of the scroll; they thicken in order */
        const local = clamp((p - i/els.length*0.72) * 3.2, 0, 1);
        const wght = Math.round(lerp(200, 800, local));
        const opsz = lerp(8, 72, local);
        w.style.fontVariationSettings = '"wght" ' + wght + ',"opsz" ' + opsz.toFixed(1);
        w.style.color = local > .5 ? "var(--ink)" : "#a8a49c";
      });
    });
  })();
})();
