...
(function(){
    const stage = $("#odoStage");
    $$(".odo", stage).forEach(el=>{
      const val = el.dataset.val, suf = el.dataset.suf;
      const drums = [];
      [...val].forEach(()=>{
        const d = document.createElement("span"); d.className = "drum";
        const u = document.createElement("u");
        for (let rep=0; rep<3; rep++) for (let n=0; n<10; n++){
          const b = document.createElement("b"); b.textContent = String(n); u.appendChild(b);
        }
        d.appendChild(u); el.appendChild(d); drums.push(u);
      });
      const s = document.createElement("span"); s.className = "suf"; s.textContent = suf; el.appendChild(s);
      el._roll = ()=> drums.forEach((u,i)=>{
        const digit = parseInt(val[i], 10);
        u.style.transitionDelay = (i*130) + "ms";
        u.style.transform = "translateY(-" + (20 + digit) + "em)";
      });
    });
    once(stage, ()=> $$(".odo", stage).forEach(el=> requestAnimationFrame(el._roll)), .45);
  })();
})();
