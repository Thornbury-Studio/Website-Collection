...
`css/` + `js/` + `img/` + `film/`, no build step, no bundler. Every figure —
spec, derived claim, idle and full-load readout, page description — comes
from `js/redline-model.js`; after changing the model, run
`node tools/bake.mjs` to re-bake the static numbers. Pictures and films are
re-derived from the licensed masters by `python tools/media.py`.
