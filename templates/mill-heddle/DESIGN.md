...
|---|---|---|
| Kvadrat's catalogue | The swatch is the product image: construction and colour carry the page, and everything around them is quiet. Numbers beside every cloth. | Their grid, their type, their palette, their photography, any of their names. |
...
**Kirkbrae Twill**, **Ettrick Herringbone**, **Hound**, **Shepherd Check**,
**Glen**, **Log Cabin**, **Rosepath**, **Yarrow Diamond**.

## The one idea: nothing is photographed

Every cloth on the site is painted by `js/weave.js` from its own draft — the
threading, the tie-up, the treadling and the colour order — at the sett the
mill weaves it. A swatch is a construction, not a picture of one. This is
what lets the site do the three things no photograph could:
...
2. **The range, drawn** — eight cards on the home page, and the range at
   length with the draft written out beside each cloth in the notation a
   weaver would recognise.
...
1. `index.html` — the loom beside the page; hero, the range, the mill, ordering
   with the enquiry form, footer with the pick count and a fringe.
2. `cloths.html` — the eight at length: cloth at 8 px a thread, the draft
   written out, a spec table, order and open-the-draft links.
